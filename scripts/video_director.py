...
from runtime.operation_confirmation import OperationConfirmationError, apply_operation_confirmation  # noqa: E402
...
def _cmd_run(args: Sequence[str]) -> int:
    ...
    try:
        result = workflow.run()
    except (OperationConfirmationError, ProductionConfigError, RuntimeError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0
...
def _cmd_confirm_operation(args: Sequence[str]) -> int:
    parser = argparse.ArgumentParser(description="Mark operation parameters as confirmed")
    parser.add_argument("config", help="path to the Video Director config")
    parser.add_argument("--note", help="optional confirmation note")
    parsed = parser.parse_args(args)
    ...
    apply_operation_confirmation(payload, approved=True, note=str(parsed.note or "").strip())
...
def _cmd_demo(args: Sequence[str]) -> int:
    ...
    config_args = [
        ...
        "--operation-confirmed",
        "--operation-note",
        "Built-in demo parameters are generated by Video Director and do not require a separate human confirmation.",
...
commands:
  ...
  confirm-operation  Mark operation parameters as confirmed
...
def main(argv: Sequence[str]) -> int:
    ...
    if command == "confirm-operation":
        return _cmd_confirm_operation(args)
